...
CStringA CUtils::DoSHA1(PBYTE buffer, UINT bufSize)
{
	...
	for(int i = 0; i < SHA1_DIGEST_VALUELEN; i++) 
	{
		...
		_itoa_s(sha1value[i], (char *)tmpBuf, 3, 16);
...
BOOL CUtils::ExportToFile(CString strPath, PBYTE buffer, UINT bufSize)
{
	CFile dstFile;

	if(PathFileExists(strPath))
	{
		if(dstFile.Open(strPath, CFile::modeReadWrite) == FALSE)
		{
			return FALSE;
		}
	}
	...
	{
		if(dstFile.Open(strPath, CFile::modeCreate | CFile::modeWrite) == FALSE)
		{
			return FALSE;
		}
	}
...
unsigned short CUtils::bigEn_2bytetoint(unsigned char *arr)  
{
	unsigned short ret = arr[1] + (arr[0] << 8);

	return ret;
}

unsigned short CUtils::littleEn_2bytetoint(unsigned char *arr)  
{
	unsigned short ret = arr[0] + (arr[1] << 8);

	return ret;
}

unsigned int CUtils::bigEn_3bytetoint(unsigned char *arr)  
{
	unsigned int ret = arr[2] + (arr[1] << 8) + (arr[0] << 16);

	return ret;
}

unsigned int CUtils::littleEn_3bytetoint(unsigned char *arr)  
{
	unsigned int ret = arr[0] + (arr[1] << 8) + (arr[2] << 16);
